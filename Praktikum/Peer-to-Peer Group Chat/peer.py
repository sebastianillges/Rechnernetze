...
class Peer():
    ...
    def __init__(self, nickname, ip, udp_port, tcp_port, server_ip, server_port, p2p_port):
        ...
        self.thread_start_p2p = threading.Thread(target=self.start_p2p, name="thread_start_p2p")
        self.thread_listen_p2p = threading.Thread(target=self.listen_p2p, name="thread_listen_p2p")
        self.thread_eval_server_response = threading.Thread # wird in listen_tcp_server() initialisiert wegen parameter
        ...
        while True:
            try:
                ...
                if not data:                                        # receiving empty messages means that the socket other side closed the socket
                    ...
                    self.thread_eval_server_response = threading.Thread(target=self.eval_msg(data), name="thread_eval_server_response").start()
                    break
            except:
                if self.LOGGEDIN:
                    break
